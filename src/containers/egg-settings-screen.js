...
export const EggSettings = ({ eggSettings, setSetting, setResults, setScreen }) => {
  const {
    applyFilters,
    eggSeeds,
    femaleAbility,
  ...
  return (
    <div>
      ...
      <Divider style={{ backgroundColor: 'rgba(255,255,255,0.4)' }} />
      <TextInput placeholder="Your TSV" onChange={tsv => saveSetting('playerTSV')(parseInt(tsv))} value={playerTSV} />
      <TextInput
      ...
      />
      <Checkbox color="#0277BD" label="Shiny Charm" checked={shinyCharm} onCheckHandler={saveSetting('shinyCharm')} />
      ...
      />
      <TextInput placeholder="Perfect IVs" onChange={saveSetting('perfectIVFilter')} value={perfectIVFilter} />
      <RadioButton
        color="#0277BD"
        onChange={saveSetting('genderFilter')}
        selected={genderFilter}
        buttonSettings={[{ label: 'No Gender' }, { label: 'Genderless' }, { label: 'Male' }, { label: 'Female' }]}
      />
      <Checkbox
        color="#0277BD"
