...
import { changeScreen, setRNGResults, setStationarySetting } from '../store/dispatchers';
import { parseList, parseIVList } from '../utils/parse-number-list';
import { pokemonFilter } from '../utils/pokemon-filters';

export const StationarySettings = ({ stationarySettings, setSetting, setResults, setScreen }) => {
  const mainStyle = {
    margin: '0px auto',
    textAlign: 'left',
    width: '320px'
  ...
  const handleGenerateEggs = () => {
    ...
    const results = applyFilters ? filter(stationaries, isPassingFilters) : stationaries;
